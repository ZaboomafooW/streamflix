...
def trim_mobile() -> None:
    ...
    text = replace_once(text, '''    private fun displayVideo(\n        video: Video,\n        server: Video.Server,\n        startPositionMs: Long? = null,\n        shouldPlay: Boolean = true,\n    ) {\n''', '''    private fun displayVideo(video: Video, server: Video.Server) {\n''', "mobile display signature")
    text = replace_once(text, "        val currentPosition = startPositionMs ?: player.currentPosition\n", "        val currentPosition = player.currentPosition\n", "mobile position capture")
    text = replace_once(text, '''        if (startPositionMs != null) {\n            player.seekTo(startPositionMs)\n        } else if (currentPosition == 0L) {\n''', '''        if (currentPosition == 0L) {\n''', "mobile explicit resume seek")
    text = replace_once(text, "        player.playWhenReady = shouldPlay\n", "        player.play()\n", "mobile play-state restore")
    for token in ("pendingPlaybackPositionMs", "pendingPlaybackShouldPlay"):
        if token in text:
            raise RuntimeError(f"mobile still contains out-of-scope continuity state: {token}")
    if "private fun displayVideo(\n" in text:
        raise RuntimeError("mobile displayVideo still has continuity-only expanded signature")
...
def trim_tv() -> None:
    ...
    for token in ("pendingPlaybackPositionMs", "pendingPlaybackShouldPlay"):
        if token in text:
            raise RuntimeError(f"tv still contains out-of-scope continuity state: {token}")
    if "private fun displayVideo(\n            video: Video,\n            server: Video.Server,\n            startPositionMs:" in text:
        raise RuntimeError("tv displayVideo still has continuity-only expanded signature")
    TV.write_text(text)
