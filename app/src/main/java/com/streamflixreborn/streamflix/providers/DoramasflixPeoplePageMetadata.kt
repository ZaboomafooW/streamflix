...
internal class DoramasflixPeoplePageMetadata {

    private val baseUrl = DoramasflixProvider.baseUrl.trimEnd('/')
    ...
    suspend fun getPeople(id: String): People? = try {
        ...
        Log.w(
            "DoramasflixPeoplePage",
            "Actor page metadata is unavailable for '$id'",
            error,
        )
        null
    }
    ...
    companion object {
        internal fun parsePeople(
            ...
            return People(
                ...
                birthday = stringValue(structuredPerson?.get("birthDate"))
                    ?: labeledValue(document, "Cumpleaños")
        ...
        private fun peopleFilmography(document: Document): List<Show> =
            document.select("h2")
                ...
                .mapNotNull(::filmographyItem)
                .distinctBy(::showIdentity)
                .toList()
        ...
        private fun filmographyItem(link: Element): Show? {
            ...
            if (!isDorama && !isVariety && !isMovie) return null
        ...
        private fun labeledValue(document: Document, label: String): String? {
            ...
            return document.getAllElements()
                ...
                .mapNotNull { text ->
                    ...
                    text.substring(separator + 1).trim().takeIf(String::isNotEmpty)
                }
                .firstOrNull()
        }
        ...
        private fun elementImage(image: Element): String? {
            ...
            val value = absolute.ifEmpty { raw }.takeIf(String::isNotEmpty) ?: return null
